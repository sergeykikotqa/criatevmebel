...
export function HeroOverlay({
  ...
  useEffect(() => {
    ...
    const exitTimer = setTimeout(() => {
      if (activeIndex < phrases.length - 1) {
        setActiveIndex((current) => (current === null ? current : ((current + 1) as 0 | 1 | 2)));
        setPhase("visible");
        return;
      }
  ...
  }, [activeIndex, exitMs, holdMs, phase, phrases.length, prefersReducedMotion]);
  ...
  return (
    ...
    >
      <div className="min-h-[3.5rem] max-w-[15rem] text-right">
        <p
          className={`relative inline-block text-[clamp(1.9rem,2.7vw,3rem)] font-semibold leading-[0.92] tracking-[-0.085em] text-white/92 transition-[opacity,transform] duration-[420ms] [transition-timing-function:cubic-bezier(0.22,1,0.36,1)] ${
            isVisible ? "translate-y-0 opacity-100" : "-translate-y-2 opacity-0"
          }`}
          style={{ ...overlayTextStyle, ...transitionStyle }}
        >
          <span>{currentPhrase ?? phrases[phrases.length - 1]}</span>
          <span
